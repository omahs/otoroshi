...
object RulesManager {
  val sections = Seq(
    RulesSection(SectionId("architecture"), Seq(
      Rule(RuleId("AR01"), weight = 25, sectionWeight = 25,
        description = "Use Event Driven Architecture to avoid polling madness and inform subscribers of an update.".some,
        advice = "Use Event Driven Architecture to avoid polling madness.".some),
      Rule(RuleId("AR02"), weight = 25, sectionWeight = 25,
        description = "API runtime close to the Consumer.".some,
        advice = "Deploy the API near the consumer".some),
      Rule(RuleId("AR03"), weight = 25, sectionWeight = 25,
        description = "Ensure the same API does not exist *.".some,
        advice = "Ensure only one API fit the same need".some),
      Rule(RuleId("AR04"), weight = 25, sectionWeight = 25,
    ...
    RulesSection(SectionId("design"), Seq(
      Rule(RuleId("DE01"), weight = 25, sectionWeight = 40,
    ...
    RulesSection(SectionId("usage"), Seq(
      Rule(RuleId("US01"), weight = 5, sectionWeight = 25,
        description = "Use query parameters for GET Methods.".some,
        advice = "Implement filters to limit which data are returned by the API (send just the data the consumer need).".some),
      Rule(RuleId("US02"), weight = 10, sectionWeight = 25,
        description = "Decomission end of life or not used APIs.".some,
        advice = "Decomission end of life or not used APIs".some),
      Rule(RuleId("US03"), weight = 10, sectionWeight = 25,
        description = "Number of API version <=2 .".some,
        advice = "Compute resources saved & Network impact reduced".some),
      Rule(RuleId("US04"), weight = 10, sectionWeight = 25,
      ...
      Rule(RuleId("US05"), weight = 20, sectionWeight = 25,
        description = "Choosing relevant data representation (user don’t need to do multiple calls) is Cx API ?.".some,
        advice = "Choose the correct API based on use case to avoid requests on multiple systems or large number of requests. Refer to the data catalog to validate the data source.".some),
      Rule(RuleId("US06"), weight = 25, sectionWeight = 25,
        description = "Number of Consumers.".some,
        advice = "Deploy an API well designed and documented to increase the reuse rate. Rate based on number of different consumers".some),
      Rule(RuleId("US07"), weight = 20, sectionWeight = 25,
    ...
    RulesSection(SectionId("log"), Seq(
      Rule(RuleId("LO01"), weight = 100, sectionWeight = 10, description = "Logs retention.".some, advice = "Align log retention period to the business need (ops and Legal)".some)
    ))
  )
}
