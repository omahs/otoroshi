...
class GreenScoreExtension(val env: Env) extends AdminExtension {
  ...
  override def adminApiRoutes(): Seq[AdminExtensionAdminApiRoute] = Seq(
    AdminExtensionAdminApiRoute(
      "GET",
      "/api/extensions/green-score/:greenscore/eco",
      false,
      (ctx, request, apk, _) => {
        implicit val ec = env.otoroshiExecutionContext
        implicit val ev = env
        ...
        for {
          ...
          val score = scores
            ...
            .map(score => score.routes.foldLeft(0)((acc, e) => {
              val optBackend = env.proxyState.route(e.routeId).map(_.backend)

              if (optBackend.nonEmpty && optBackend.get.targets.nonEmpty) {
                val backend = optBackend.get
                val target = backend.targets.head.toTarget
                val backendId = target.scheme + target.host + backend.root

                acc + ecoMetrics.computeBackend(backendId)
              } else {
                acc
              }
            }))
    ...
    AdminExtensionAdminApiRoute(
      ...
      false,
      (_, _, _, _) => {
        Results.Ok(GreenScoreConfig(sections = RulesManager.sections, dynamicSections = DynamicRulesSections()).json).vfuture
      }
    )
  )
